...
class IOSLog extends EventEmitter {
  ...
  async startCaptureRealDevice () {
    ...
    if (!_.isEmpty(this.deviceLogPath)) {
      try {
        logger.debug('Real device log : deviceconsole to deviceLogPath!');
        ...
        this.grep.stdout.on('data',(data) => {
          // logger.info(`Error when writing device log ! ${data}`);
          fse.appendFile(devicelogpath, data, function (err) {
